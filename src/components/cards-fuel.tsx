    ...
    <>
      {isMobile ? (
        <div className="card-container font-montserrat mt-1 h-100 flex-column w-100">
          ...
          {t.fuelcard.cardsfuel.map(
            ({ strong, icon }: ICard, key: number) => (
              <Card key={key} className="mx-3 w-100 bg-light text-dark h-100 d-flex align-items-center">
        ...
        <div className="card-container font-montserrat mt-1 h-100 flex-column w-100">
          ...
          {t.fuelcard.cardsfuel.map(
            ({ strong, icon }: ICard, key: number) => (
              <Card key={key} className="mx-3 mb-4 w-100 bg-light text-dark h-100 d-flex align-items-center">
              <Card.Body className="text-start d-flex align-items-left">
